...
def plot_word_correlation_scm(word_correlation_dir, variant, layers='all', perplexity=30, result_filename='default'): 
    with open(os.path.join(DATA_DIR, 'warm.txt'), 'r') as rf:
        warm_words = rf.readlines()
        warm_words = [w.strip() for w in warm_words]
    with open(os.path.join(DATA_DIR, 'cold.txt'), 'r') as rf:
        cold_words = rf.readlines()
        cold_words = [w.strip() for w in cold_words]
    with open(os.path.join(DATA_DIR, 'competent.txt'), 'r') as rf:
        competent_words = rf.readlines()
        competent_words = [w.strip() for w in competent_words]
    with open(os.path.join(DATA_DIR, 'incompetent.txt'), 'r') as rf:
        incompetent_words = rf.readlines()
        incompetent_words = [w.strip() for w in incompetent_words]
    with open(os.path.join(DATA_DIR, 'neutral.txt'), 'r') as rf:
        neutral_words = rf.readlines()
        neutral_words = [w.strip() for w in neutral_words]
    word_embedding = torch.load(os.path.join(word_correlation_dir, 'word_embedding_30.bin'))
    ...
    for i, w in enumerate(words):
        if w in warm_words:
            warm_i.append(i)
        elif w in cold_words:
            cold_i.append(i)
        elif w in competent_words:
            comp_i.append(i)
        elif w in incompetent_words:
            inco_i.append(i)
        elif w in neutral_words:
            n_i.append(i)
        else:
            print(f'Bad word: {w}')
    warm_i = np.array(warm_i)
    cold_i = np.array(cold_i)
    comp_i = np.array(comp_i)
    inco_i = np.array(inco_i)
    n_i = np.array(n_i)
    vectors = np.array(vectors)
    tsne = TSNE(n_components=2, perplexity=perplexity, random_state=46)
    result = tsne.fit_transform(vectors)
    ...
    Y = result[..., 1]
    plt.figure()
    if variant in ['scm', 'warmth']: 
        plt.scatter(X[warm_i], Y[warm_i], c='xkcd:dark pink')
        plt.scatter(X[cold_i], Y[cold_i], c='xkcd:goldenrod')
    if variant in ['scm', 'competence']: 
        plt.scatter(X[comp_i], Y[comp_i], c='xkcd:robin\'s egg blue')
        plt.scatter(X[inco_i], Y[inco_i], c='xkcd:olive green')
    plt.scatter(X[n_i], Y[n_i], c='xkcd:beige')
    for w, x, y in zip(words, X, Y):
        plt.annotate(w, (x, y), fontsize=5)
    plt.title('t-SNE')
