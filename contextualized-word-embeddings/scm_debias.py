...
def load_and_cache_examples(data, args, tokenizer):
    # first_train_dataset = create_dataset(data['scm'][0]['train'], LineByLineTextDataset)
    # first_dev_dataset = create_dataset(data['scm'][0]['dev'], LineByLineTextDataset)
    # second_train_dataset = create_dataset(data['scm'][1]['train'], LineByLineTextDataset)
    # second_dev_dataset = create_dataset(data['scm'][1]['dev'], LineByLineTextDataset)
    # train_dataset = create_dataset(data['train'], LineByLineTextDataset)
    # dev_dataset = create_dataset(data['dev'], LineByLineTextDataset)
    # # logger.info(first_train_dataset)
    # return {
    #     'train': train_dataset, 'dev': dev_dataset, 
    #     'train_0': first_train_dataset, 'dev_0': first_dev_dataset, 
    #     'train_1': second_train_dataset, 'dev_1': second_dev_dataset, 
    # }
...
def split_data_scm(attributes_examples, attributes_labels, neutral_examples, neutral_labels, args): 
    ...
    train_neutral_example = neutral_examples[args.dev_data_size:]
    train_neutral_label = neutral_labels[args.dev_data_size:]
    dev_neutral_example = neutral_examples[:args.dev_data_size]
    dev_neutral_label = neutral_labels[:args.dev_data_size]

    scm_data = {
        'train_warmth': {
            'example': {
                'attribute0': attributes_examples[0][args.dev_data_size:], 
                'attribute1': attributes_examples[1][args.dev_data_size:], 
                'neutral': train_neutral_example if args.use_neutral else attributes_examples[2][args.dev_data_size:]+attributes_examples[3][args.dev_data_size:], 
            }, 
            'label': {
                'attribute0': attributes_labels[0][args.dev_data_size:], 
                'attribute1': attributes_labels[1][args.dev_data_size:], 
                'neutral': train_neutral_label if args.use_neutral else attributes_labels[2][args.dev_data_size:]+attributes_labels[3][args.dev_data_size:], 
            }, 
        }, 
        'train_competence': {
            'example': {
                'attribute0': attributes_examples[2][args.dev_data_size:], 
                'attribute1': attributes_examples[3][args.dev_data_size:], 
                'neutral': train_neutral_example if args.use_neutral else attributes_examples[0][args.dev_data_size:]+attributes_examples[1][args.dev_data_size:], 
            }, 
            'label': {
                'attribute0': attributes_labels[2][args.dev_data_size:], 
                'attribute1': attributes_labels[3][args.dev_data_size:], 
                'neutral': train_neutral_label if args.use_neutral else attributes_labels[0][args.dev_data_size:]+attributes_labels[1][args.dev_data_size:], 
            }, 
        }, 
        'dev_warmth': {
            'example': {
                'attribute0': attributes_examples[0][:args.dev_data_size], 
                'attribute1': attributes_examples[1][:args.dev_data_size], 
                'neutral': dev_neutral_example if args.use_neutral else attributes_examples[2][:args.dev_data_size]+attributes_examples[3][:args.dev_data_size], 
            }, 
            'label': {
                'attribute0': attributes_labels[0][:args.dev_data_size], 
                'attribute1': attributes_labels[1][:args.dev_data_size], 
                'neutral': dev_neutral_label if args.use_neutral else attributes_labels[2][:args.dev_data_size]+attributes_labels[3][:args.dev_data_size], 
            }, 
        }, 
        'dev_competence': {
            'example': {
                'attribute0': attributes_examples[2][:args.dev_data_size], 
                'attribute1': attributes_examples[3][:args.dev_data_size], 
                'neutral': dev_neutral_example if args.use_neutral else attributes_examples[0][:args.dev_data_size]+attributes_examples[1][:args.dev_data_size], 
            }, 
            'label': {
                'attribute0': attributes_labels[2][:args.dev_data_size], 
                'attribute1': attributes_labels[3][:args.dev_data_size], 
                'neutral': dev_neutral_label if args.use_neutral else attributes_labels[0][:args.dev_data_size]+attributes_labels[1][:args.dev_data_size], 
            }, 
        }, 
    }
    return scm_data
...
def split_data(attributes_examples, attributes_labels, neutral_examples, neutral_labels, args):
    ...
    # data['scm'] = [{'train': {'example': {}, 'label': {}}, 'dev': {'example': {}, 'label': {}}} for _ in range(2)]
    # # data['scm'][0]['train']['example'][f'attribute0']
    # for dim_i, (dim, att_0, att_1) in enumerate([('warmth', 0, 1), ('competency', 2, 3)]): 
    #     for split in ['train', 'dev']: 
    #         for data_part in ['example', 'label']: 
    #             data['scm'][dim_i][split][data_part][f'attribute0'] = data[split][data_part][f'attribute{att_0}']
    #             data['scm'][dim_i][split][data_part][f'attribute1'] = data[split][data_part][f'attribute{att_1}']
...
def train(args, data, datasets, model: PreTrainedModel, original_model, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    # logger.info(datasets['train'].keys())
    warmth_dev_datasets = datasets['warmth']['dev']
    competence_train_datasets = datasets['competence']['train']
    competence_dev_datasets = datasets['competence']['dev']

    logger.info('Init dataloaders first time')
    warmth_train_dataloaders, warmth_train_example_num, warmth_train_distribution = create_dataloader(args, warmth_train_datasets, tokenizer, train=True)
    warmth_dev_dataloaders, warmth_dev_example_num, warmth_dev_distribution = create_dataloader(args, warmth_dev_datasets, tokenizer, train=False)
    competence_train_dataloaders, competence_train_example_num, competence_train_distribution = create_dataloader(args, competence_train_datasets, tokenizer, train=True)
    competence_dev_dataloaders, competence_dev_example_num, competence_dev_distribution = create_dataloader(args, competence_dev_datasets, tokenizer, train=False)
    logger.info('Done init dataloaders first time')
    ...
    train_iter_num = sum([len(dataloader) for dataloader in warmth_train_dataloaders.values()])
    ...
    logger.info("Num examples = %d", warmth_train_example_num)
    ...
    def get_hiddens_of_model(input, input_attention_mask):
        ...
        if args.model_type == 'roberta':
            ...
            if args.algorithm in ['ADEPT', 'ADEPT-DPCEloss']:
                hiddens = model(input, input_attention_mask).hidden_states
            elif args.algorithm in ['ADEPT-finetuning','DPCE']:
    ...
    def attribute_vector_example():
        ...
        #     d = 3
        # elif args.bias == 'warmth':
        #     d = 2
        # elif args.bias == 'competence':
        #     d = 2
        d = {
            'gender': 2, 
            'religion': 3, 
        ...
        for key in distribution:
            if key != 'neutral':
                ...
                if labels.size(1) > 1:
                    ...
                    onehot = torch.eye(hiddens.size(1))
                    onehot = onehot.to(args.device)
                    onehot = onehot[labels].view(hiddens.size(0), -1, 1, 1)
                attributes_hiddens[key].append(torch.sum(hiddens * onehot, 1) / labels.size(1))

        attribute_size = len(data[f'train_{scm_dim_name}']['example'])
    ...
    def forward(attributes_hiddens, dataloaders, key):
        ...
        if args.loss_target == 'sentence' or labels is None:
            ...
            if 'neutral' != key:
                ...
                elif args.algorithm == 'DPCE' or args.algorithm=='ADEPT-DPCEloss':
    ...
    def evaluate(model, attributes_hiddens, dev_dataloaders, prefix=""):
        ...
        logger.info("Num examples = %d", warmth_dev_example_num)
    ...
    for _ in train_iterator:
        # for scm_dim_name, (train_datasets, train_dataloaders, train_example_num, train_distribution, 
        #                 dev_datasets, dev_dataloaders, dev_example_num, dev_distribution) in [
        #                     ('warmth', [warmth_train_datasets, warmth_train_dataloaders, warmth_train_example_num, warmth_train_distribution, 
        #                                 warmth_dev_datasets, warmth_dev_dataloaders, warmth_dev_example_num, warmth_dev_distribution]), 
        #                     ('competence', [competence_train_datasets, competence_train_dataloaders, competence_train_example_num, competence_train_distribution, 
        #                                 competence_dev_datasets, competence_dev_dataloaders, competence_dev_example_num, competence_dev_distribution]), 
        #                 ]: 
        for i, scm_dim_name in enumerate(['warmth', 'competence']): 
            
            train_datasets = datasets[scm_dim_name]['train']
            dev_datasets = datasets[scm_dim_name]['train']

            train_dataloaders, train_example_num, train_distribution = create_dataloader(args, train_datasets, tokenizer, train=True)
            dev_dataloaders, dev_example_num, dev_distribution = create_dataloader(args, dev_datasets, tokenizer, train=False)

            random.shuffle(train_distribution)
            epoch_iterator = tqdm(train_distribution, desc=f"Iteration ({scm_dim_name})", disable=args.local_rank not in [-1, 0])
...
def main():
    ...
    if model_args.model_name_or_path:
        if args.algorithm == 'ADEPT' or args.algorithm=='ADEPT-DPCEloss':
    ...
    logger.info(len(data['attributes_examples']))
    logger.info(type(data['attributes_examples']))
    logger.info(len(data['attributes_examples'][0]))
    logger.info(len(data['attributes_examples'][3]))
    attributes_labels = data['attributes_labels']
    logger.info(len(data['attributes_labels']))
    logger.info(len(data['attributes_labels'][0]))
    logger.info(len(data['attributes_labels'][3]))
    neutral_examples = data['neutral_examples']
    neutral_labels = data['neutral_labels']

    scm_split_data = split_data_scm(attributes_examples, attributes_labels, neutral_examples, neutral_labels, args)

    # if 'neutral_labels' in data:
    #     logger.info('neutral_labels in data')
    #     neutral_labels = data['neutral_labels']
    #     logger.info(f'neutral_labels: {len(neutral_labels)}, neutral_examples: {len(neutral_examples)}')
    ...
    datasets = load_and_cache_examples(scm_split_data, args, tokenizer)
    ...
    train(args, scm_split_data, datasets, model, original_model, tokenizer)
